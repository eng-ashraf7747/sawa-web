...
import { useNewContactMessagesCount } from "@/hooks/useContactMessages";
...
export default function AdminLayout({ children, title }: AdminLayoutProps) {
  ...
  const { count: newMessagesCount } = useNewContactMessagesCount();
      ...
      <aside className={`fixed top-0 right-0 h-full bg-[#1e293b] text-white z-30 flex flex-col transition-all duration-300 ${sidebarOpen ? "w-64" : "w-16"}`}>
        <div className="flex items-center justify-between px-4 py-5 border-b border-white/10">
          ...
          <button
            ...
            aria-label="طي أو فتح القائمة الجانبية"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <nav className="flex-1 py-4 overflow-y-auto">
          {navItems.map((item) => {
            ...
            const badgeCount = item.id === "messages" ? newMessagesCount : 0;

            return (
              <div key={item.id} className="px-2 mb-1 relative">
                {item.enabled ? (
                  <Link href={item.href} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 ${isActive ? "bg-[#1a3c6e] text-white shadow-md" : "text-white/70 hover:bg-white/10 hover:text-white"}`}>
                    <span className={`relative ${isActive ? "text-[#c9a84c]" : ""}`}>
                      {item.icon}
                      {!sidebarOpen && badgeCount > 0 && (
                        <span className="absolute -top-0.5 -left-0.5 w-2 h-2 bg-[#c9a84c] rounded-full" />
                      )}
                    </span>
                    {sidebarOpen && (
                      <span className="text-sm font-medium flex items-center flex-1 truncate">
                        {item.label}
                        {badgeCount > 0 && (
                          <span className="mr-auto bg-[#c9a84c] text-[#1e293b] text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                            {badgeCount}
                          </span>
                        )}
                      </span>
                  ...
                  <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-white/30 cursor-not-allowed" title="قريباً">
                    ...
                    {sidebarOpen && (
                      <div className="flex items-center justify-between flex-1 min-w-0">
                        ...
                        <span className="text-[10px] bg-white/10 px-1.5 py-0.5 rounded-full flex-shrink-0">قريباً</span>
                      </div>
                    )}
                  </div>
                )}
